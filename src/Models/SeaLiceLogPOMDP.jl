...
# -------------------------
"State representing sea lice level (log space), season, farm biomass, and treatment cooldown."
struct SeaLiceLogState
	SeaLiceLevel::Float64   # log(adult lice per fish)
...
"Observation: noisy lice count (log space) plus perfectly observed season, biomass, and cooldown."
...
# -------------------------
"Sea lice POMDP in log space with 4D state: (log lice level, season, biomass, treatment cooldown)."
@with_kw struct SeaLiceLogPOMDP <: POMDP{SeaLiceLogState, Action, SeaLiceLogObservation}
    ...
    # Regulation parameters — season-dependent thresholds (in raw space)
    ...
    # Count bounds (log space)
    sea_lice_bounds::Tuple{Float64, Float64} = (log(1e-3), log(10.0)) # (-6.91, 2.30)
...
# POMDPs.jl Interface
# -------------------------
function POMDPs.states(pomdp::SeaLiceLogPOMDP)
    state_list = SeaLiceLogState[]
    sizehint!(state_list, pomdp.n_lice * pomdp.n_season * pomdp.n_biomass * pomdp.n_cooldown)
    for c in 0:1
        for b in pomdp.biomass_range
            for s in 1:4
                for l in pomdp.sea_lice_range
                    push!(state_list, SeaLiceLogState(l, s, b, c))
                end
            end
        end
    end
...
function POMDPs.observations(pomdp::SeaLiceLogPOMDP)
    obs_list = SeaLiceLogObservation[]
    sizehint!(obs_list, pomdp.n_lice * pomdp.n_season * pomdp.n_biomass * pomdp.n_cooldown)
    for c in 0:1
        for b in pomdp.biomass_range
            for s in 1:4
                for l in pomdp.sea_lice_range
                    push!(obs_list, SeaLiceLogObservation(l, s, b, c))
                end
            end
        end
    end
...
function POMDPs.transition(pomdp::SeaLiceLogPOMDP, s::SeaLiceLogState, a::Action)
    ...
    # === 3. Sea lice transition (log space, temperature from season) ===
    ...
    lice_dist = truncated(Normal(μ, log_adult_sd), pomdp.sea_lice_bounds...)
    lice_probs = discretize_values(lice_dist, pomdp.sea_lice_range)

    # === 4. Biomass transition (growth model, uses raw lice level) ===
    raw_adult = exp(s.SeaLiceLevel)
    lice_growth_factor = 1.0 / (1.0 + exp(5.0 * (raw_adult - 0.5)))
...
# -------------------------
# Observation function: sea lice measured with negative binomial (log space);
...
function POMDPs.observation(pomdp::SeaLiceLogPOMDP, a::Action, sp::SeaLiceLogState)
    ...
    # If full observability solver, return exact state
    if pomdp.full_observability_solver
        all_obs = POMDPs.observations(pomdp)
        target_idx = obsindex(pomdp, SeaLiceLogObservation(sp.SeaLiceLevel, sp.Season, sp.BiomassLevel, sp.Cooldown))
    ...
    k = max(1e-9, pomdp.n_sample * pomdp.ρ_adult)
    r, p = nb_params_from_mean_k(μ_total_adult, k)
    nb = NegativeBinomial(r, p)

    # Build CDF at each lice grid value (log space → raw space threshold)
    ...
    if lice_total <= 0 || !isfinite(lice_total)
        μ_adult_log = log(max(μ_total_adult / pomdp.n_sample, 1e-12))
        idx = findmin(abs.(lice_range .- μ_adult_log))[2]
        lice_probs .= 0.0
        lice_probs[idx] = 1.0
    else
...
function POMDPs.reward(pomdp::SeaLiceLogPOMDP, s::SeaLiceLogState, a::Action, sp::SeaLiceLogState, o::SeaLiceLogObservation)
    ...
    # === 2. REGULATORY PENALTY (MNOK) — based on OBSERVATION (converted to natural space), season-dependent ===
    reg_limit = pomdp.season_regulation_limits[s.Season]
    observed_adult_level = exp(o.SeaLiceLevel)
    regulatory_penalty = observed_adult_level > reg_limit ? pomdp.regulatory_violation_cost_MNOK : 0.0
...
function POMDPs.initialstate(pomdp::SeaLiceLogPOMDP)
    ...
    # Sea lice distribution in log space (uncertain)
